...
import { TrendingUp, BarChart2, Users, Target, Award, ArrowUpRight, ArrowDownRight } from "lucide-react";
import CardWrp from "@/components/CardWrp";

const kpis = [
  { label: "Total Applications",  value: "142", change: "+18%", trend: "up",   icon: BarChart2, color: "text-orange-400", bg: "bg-orange-500/10" },
  { label: "Response Rate",       value: "23%", change: "+5%",  trend: "up",   icon: Users,     color: "text-orange-300", bg: "bg-orange-400/10" },
  { label: "Interview Conversion",value: "15%", change: "-2%",  trend: "down", icon: Target,    color: "text-amber-400",  bg: "bg-amber-500/10"  },
  { label: "Offer Rate",          value: "8%",  change: "+3%",  trend: "up",   icon: Award,     color: "text-orange-200", bg: "bg-orange-300/10" },
];
...
const topCompanies = [
  ...
  { name: "Vercel",  applied: 2, status: "Offer",     statusColor: "text-orange-400",  statusBg: "bg-orange-400/10"  },
          ...
          <CardWrp className="mt-0">
                ...
                <div key={d.month} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full flex items-end gap-0.5 h-40">
                    <div className="flex-1 rounded-t-md transition-all duration-700"
                      style={{ height: `${(d.apps / maxApps) * 100}%`, background: "linear-gradient(to top, #E64A19, #FF7043)" }} />
                    <div className="flex-1 rounded-t-md transition-all duration-700"
                      style={{ height: `${(d.interviews / maxApps) * 100}%`, background: "linear-gradient(to top, #FF8A65, #FFAB91)" }} />
                  </div>
            ...
            <div className="flex items-center gap-5 mt-4">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-sm" style={{ background: "#FF7043" }} />
                <span className="text-zinc-400 text-xs">Applications</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-sm" style={{ background: "#FFAB91" }} />
        ...
        <Grid size={{ xs: 12, lg: 5 }}>
          <CardWrp className="mt-0">
            ...
            <div className="flex flex-col gap-3">
              {topCompanies.map((co) => (
                <div key={co.name} className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] hover:bg-white/[0.04] transition-colors">
                  ...
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${co.statusBg} ${co.statusColor}`}>{co.status}</span>
                </div>
              ))}
            </div>
          </CardWrp>
